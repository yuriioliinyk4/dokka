...
//The latter, lang, identifies the language of the code snippet, adding a CSS class language-<value of lang> to the <code> tag of the output.
        ...
        val processedSnippet = when {
            ...
            inlineSnippet != null -> {
                parseSnippet(inlineSnippet.map { it.text })
            }
     ...
     */
    private fun parseSnippet(lines: List<String>, externalSnippetRegionName: String? = null): String {
        ...
        for ((idx, line) in mutableLines.withIndex()) {
            ...
            val lineWithMarkup = line.applyMarkup(activeRegions.mapNotNull { it.operation }).trimEnd()
            ...
            if (markupSpec?.endsWith(":") == true) {
                if (idx + 1 >= mutableLines.size) {
                    logger.error("@snippet: don't place markup comment with ending `:` at the last line of snippet")
                    break
                }
                mutableLines[idx + 1] = mutableLines[idx + 1] + "${markupSpecMatch.groupValues[1]} $markupSpec"
                if (snippetBodyStarted) {
                    result.addIfNotBlank(lineWithMarkup.replace(MARKUP_SPEC, "").trimEnd())
                }
                continue
            }
            ...
            if (snippetBodyStarted) {
                if (markupSpec != null) {
                    val cleanedLine = lineWithMarkup.replace(MARKUP_SPEC, "").trimEnd()
                    ...
                    when (markupTagName) {
                        "start" -> {
                            ...
                            if (regionName == null) {
                                logger.error("@snippet: tag @start without specified region attribute")
                                continue
                            }
                        ...
                        "end" -> {
                            ...
                            if (regionName != null) {
                                if (regionName == externalSnippetRegionName) {
                                    result.addIfNotBlank(cleanedLine)
                                    snippetBodyStarted = false
                                    break
                                }
                                var regionFound = false
                                for (activeRegion in activeRegions) {
                                    if (activeRegion.regionName == regionName) {
                                        activeRegions.remove(activeRegion)
                                        regionFound = true
                                    }
                                }
                                if (!regionFound) logger.error("@snippet: invalid region \"$regionName\" in @end")
                            } else {
                                ...
                                if (lastActiveRegion != null) {
                                    ...
                                    logger.error("@snippet: `@end` tag without a matching start of the region")
                                }
                            }
                        ...
                        "highlight", "replace", "link" -> {
                            val operation = when (markupTagName) {
                                ...
                                "link" -> createLinkOperation(attributes)
                                else -> { nothing -> nothing }
                            }

                            val lineWithAppliedOperation = operation(cleanedLine).trimEnd()
                            ...
                            logger.error("@snippet: unrecognized tag @$markupTagName in markup comment")
                        }
                    }
        ...
        if (activeRegions.isNotEmpty()) {
            ...
            logger.error("@snippet: snippet body contains unclosed regions: $names")
        }
    ...
    private fun createHighlightOperation(attributes: Map<String, String?>): MarkupOperation {
        ...
        val (startTag, endTag) = when (type) {
            ...
            "highlighted" -> "<mark>" to "</mark>"
                ...
                logger.error("@snippet: invalid argument for `@highlight` type $type")
                "" to ""
            }
        }

        fun String.addTag() = "$startTag$this$endTag"
        ...
        return { line ->
            ...
            if (substring != null) result = result.replace(substring, substring.addTag())
            if (regex != null) result = result.replace(regex) { match -> match.value.addTag() }
    ...
    private fun createLinkOperation(attributes: Map<String, String?>): MarkupOperation {
        val substring = attributes["substring"]
        val regex = attributes["regex"]?.toRegex()
    ...
    private fun String.applyMarkup(markupOperations: List<MarkupOperation>): String =
        markupOperations.fold(this) { acc, op -> op(acc) }
    ...
    private fun getMarkupAttributes(markupSpec: String, tagName: String): Map<String, String?> =
        ATTRIBUTE.findAll(
            ...
            if (ALLOWED_ATTRS[tagName]?.contains(attributeName) != true) {
                logger.error("@snippet: invalid attribute $attributeName used in @$tagName tag")
                null
            } else {
